...
#include <omp.h>
using namespace std;using boost::multiprecision::uint128_t;using boost::multiprecision::uint512_t;using u64=uint64_t;using u128n=__uint128_t;
struct FE{int rank,b;uint8_t e;};struct BI{u64 q;int maxe;vector<u64>pw;};struct CTX{int n,B;vector<uint128_t>P;vector<vector<FE>>F;vector<BI>bs;vector<vector<vector<u64>>>inv;};struct T3C{int rank,b,e,src;u64 r;};
static uint128_t parse128(const string&s){uint128_t x=0;for(char c:s)x=x*10+(c-'0');return x;}static u64 invmod(u64 a,u64 m){__int128 t=0,nt=1,r=m,nr=a%m;while(nr){u64 q=r/nr;__int128 z=t-(__int128)q*nt;t=nt;nt=z;z=r-(__int128)q*nr;r=nr;nr=z;}if(r!=1)return 0;t%=m;if(t<0)t+=m;return(u64)t;}static vector<pair<u64,int>>fac(uint128_t n){vector<pair<u64,int>>f;for(u64 q=2;q<=100000&&uint128_t(q)*q<=n;q+=(q==2?1:2)){if(n%q)continue;int e=0;do{n/=q;e++;}while(n%q==0);f.push_back({q,e});}if(n>1)f.push_back({n.convert_to<u64>(),1});return f;}
static bool crtstep(uint512_t&x,uint512_t&M,const uint512_t&B,u64 m,u64 r){if(!r&&m!=1)return false;if(M>B){return (x%m).convert_to<u64>()==r;}u64 xm=(x%m).convert_to<u64>(),Mm=(M%m).convert_to<u64>(),iv=invmod(Mm,m);if(!iv)return false;u64 k=(u128n)((r+m-xm)%m)*iv%m;x+=M*k;M*=m;return !(M>B&&x>B);}
static int buildT3(const CTX&C,int j,int t,int u,T3C*out){size_t a=0,b=0,c=0;int n=0;while(a<C.F[j].size()||b<C.F[t].size()||c<C.F[u].size()){int rank=INT_MAX;if(a<C.F[j].size())rank=min(rank,C.F[j][a].rank);if(b<C.F[t].size())rank=min(rank,C.F[t][b].rank);if(c<C.F[u].size())rank=min(rank,C.F[u][c].rank);int ej=0,et=0,eu=0,base=-1;if(a<C.F[j].size()&&C.F[j][a].rank==rank){ej=C.F[j][a].e;base=C.F[j][a].b;a++;}if(b<C.F[t].size()&&C.F[t][b].rank==rank){et=C.F[t][b].e;base=C.F[t][b].b;b++;}if(c<C.F[u].size()&&C.F[u][c].rank==rank){eu=C.F[u][c].e;base=C.F[u][c].b;c++;}int e=max({ej,et,eu}),s=(ej==e?0:(et==e?1:2));u64 m=C.bs[base].pw[e],r=s==0?(u128n)C.inv[base][e][t]*C.inv[base][e][u]%m:s==1?(u128n)C.inv[base][e][j]*C.inv[base][e][u]%m:(u128n)C.inv[base][e][j]*C.inv[base][e][t]%m;out[n++]={rank,base,e,s,r};}return n;}
static bool crtT3(const CTX&C,int j,int t,int u,const uint512_t&B,T3C*out,int&nc,uint512_t&x,uint512_t&M){nc=buildT3(C,j,t,u,out);x=0;M=1;for(int z=0;z<nc;z++){u64 m=C.bs[out[z].b].pw[out[z].e];if(!crtstep(x,M,B,m,out[z].r))return false;}return x<=B;}
