#!/usr/bin/env python3
"""Run one single-threaded boundary search with accepted event clauses."""
...
def load_event_bundle(spec):
    path = Path(spec["event_cut_path"])
    if sha256(path) != spec["event_cut_sha256"]:
        raise ValueError("event-cut file identity mismatch")
    bundle = json.loads(path.read_text(encoding="utf-8"))
    canonical = dict(bundle)
    claimed = canonical.pop("canonical_outcome_sha256")
    actual = hashlib.sha256(
        (json.dumps(canonical, sort_keys=True, separators=(",", ":")) + "\n").encode()
    ).hexdigest()
    if (
        bundle["schema"] != "boundary-exact-event-cuts-v1"
        or actual != claimed
        or claimed != spec["event_cut_outcome_sha256"]
        or bundle["cut_count"] != len(bundle["cuts"])
    ):
        raise ValueError("event-cut contract mismatch")
    return bundle


def add_event_cut(model, active, cut, monomial_cache, cut_index):
    required = cut["required_active_variables"]
    alternatives = cut["escape_alternative_monomials"]
    if (
        required != sorted(set(required))
        or any(not 0 <= index < contract.ACTIVE_VARIABLES for index in required)
        or not alternatives
    ):
        raise ValueError("invalid event cut")
    literals = [active[index].Not() for index in required]
    for monomial in alternatives:
        if monomial != sorted(set(monomial)) or any(
            not 0 <= index < contract.ACTIVE_VARIABLES for index in monomial
        ):
            raise ValueError("invalid event escape monomial")
        literals.append(
            add_monomial(model, active, monomial, monomial_cache, f"e{cut_index}")
        )
    model.add_bool_or(literals)
...
def main():
    ...
    if spec["schema"] != "run-071-boundary-event-v1":
    ...
    if (
        len(spec["searches"]) != 72
        or spec["physical_jobs"] != 18
        or spec["logical_workers_per_job"] != 4
        or any(sum(item["group"] == group for item in spec["searches"]) != 4 for group in range(18))
    ):
        raise ValueError("wrong search coverage")
    event_bundle = load_event_bundle(spec)

    model = cp_model.CpModel()
    active = [model.new_bool_var(f"a_{index}") for index in range(contract.ACTIVE_VARIABLES)]
    monomial_cache = {}
    for cut_index, cut in enumerate(event_bundle["cuts"]):
        add_event_cut(model, active, cut, monomial_cache, cut_index)
    ...
    tie_weights = [
        1 + ((index * 1103515245 + search["seed"]) % 7)
    ...
    while rounds < spec["max_rounds"]:
        ...
        for threat in replay["threats"]:
            add_unique_cut(model, active, threat, monomial_cache, dynamic_cuts)
            dynamic_cuts += 1

    result = {
        "schema": "run-071-boundary-event-result-v1",
        "evidence_level": "bounded constructive support search with accepted exact-event clauses",
        "search": search,
        "status": status_name,
        "rounds": rounds,
        "dynamic_cuts": dynamic_cuts,
        "exact_event_cuts": event_bundle["cut_count"],
        ...
        "scope_warning": "A survivor is a necessary support candidate, not a coefficient solution; negative bounded statuses are not no-go theorems.",
    }
