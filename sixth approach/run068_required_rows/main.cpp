...
class AdaptiveScreen {
 ...
 private:
      ...
      if (Pure(state)) {
        ...
        for (const Matching& matching : matchings_) {
          ...
          for (int edge : matching) {
            values.push_back(support_[edge][state[0]][state[0]]);
          }
          row_terms.push_back(Conjunction(&model_, values));
          ++required_term_variables_;
        }
        sat::LinearExpr count;
        for (sat::BoolVar term : row_terms) count += term;
        model_.AddGreaterOrEqual(count, 1);
        required_terms_[state[0]] = std::move(row_terms);
        continue;
      }
      std::vector<sat::BoolVar> row_terms;
  ...
  void WriteOutput() const {
    ...
    out << "  \"required_uniform_targets\": true,\n";
    out << "  \"required_rows\": 3,\n";
    out << "  \"required_term_variables\": " << required_term_variables_ << ",\n";
    ...
    if (Feasible(last_response_)) {
      ...
      out << "],\n  \"uniform_matching_counts\": [";
      for (int colour = 0; colour < kColours; ++colour) {
        if (colour) out << ',';
        int count = 0;
        for (sat::BoolVar term : required_terms_[colour]) {
          count += Value(last_response_, term);
        }
        out << count;
      }
      out << "],\n  \"anchors\": [";
  ...
  std::array<std::vector<sat::BoolVar>, kColours> required_terms_;
  ...
  int required_term_variables_ = 0;
