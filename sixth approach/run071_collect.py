#!/usr/bin/env python3
"""Collect, replay, and compact all run-071 search records."""
...
def main():
    ...
    if (
        spec["schema"] != "run-071-boundary-event-v1"
        or len(spec["searches"]) != 72
        or spec["physical_jobs"] != 18
        or spec["logical_workers_per_job"] != 4
    ):
        raise ValueError("wrong immutable spec")
    event_path = Path(spec["event_cut_path"])
    if sha256(event_path) != spec["event_cut_sha256"]:
        raise ValueError("event-cut identity mismatch")
    expected = {item["id"]: item for item in spec["searches"]}
    results = []
    seen = set()
    contract_path = Path(__file__).with_name("run070_contract.py")
    worker_path = Path(__file__).with_name("run071_worker.py")
    for path in sorted(args.input_root.rglob("result.json")):
        result = json.loads(path.read_text(encoding="utf-8"))
        if result["schema"] != "run-071-boundary-event-result-v1":
        ...
        if result["worker_sha256"] != sha256(worker_path):
            raise ValueError(f"worker identity mismatch for {search_id}")
        if result["event_cut_sha256"] != sha256(event_path):
            raise ValueError(f"event-cut identity mismatch for {search_id}")
        if result["exact_event_cuts"] != 2:
            raise ValueError(f"event-cut coverage mismatch for {search_id}")
    ...
    for result in results:
        ...
        digest = hashlib.sha256(
            json.dumps(active, separators=(",", ":")).encode()
        ).hexdigest()
    ...
    survivors = sorted(
        survivor_map.values(),
        key=lambda item: (item["selected_count"], item["selected_sha256"]),
    )
    ...
    summary = {
        "schema": "run-071-boundary-event-collection-v1",
        ...
        "event_cut_outcome_sha256": spec["event_cut_outcome_sha256"],
        "exact_event_cuts": 2,
        "expected_searches": 72,
        ...
        "minimum_survivor_support": min(
            (item["selected_count"] for item in survivors), default=None
        ),
        "scope_warning": "Support survivors still require exact coefficient analysis; bounded negative statuses are not infeasibility certificates.",
    }
    ...
    shutil.copyfile(event_path, args.output_dir / "input-event-cuts.json")
    files = [
        args.output_dir / "summary.json",
        args.output_dir / "survivors.json",
        args.output_dir / "results.json.gz",
        args.output_dir / "input-spec.json",
        args.output_dir / "input-event-cuts.json",
    ]
