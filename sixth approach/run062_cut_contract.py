#!/usr/bin/env python3
"""Check that the compiled version-2 event table matches its bundle."""
...
def render(bundle_path: Path) -> str:
    ...
    result = [
        ...
        f"inline constexpr std::array<Cut, {len(bundle['cuts'])}> kVersion2{{{{\n",
    ]
    for index, cut in enumerate(bundle["cuts"]):
        assert cut["id"] == f"exact-cut-v2-{index}"
