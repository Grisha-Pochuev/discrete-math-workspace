#!/usr/bin/env python3
"""Synthetic contract for the single incomplete run-065 cell."""
...
BASE_SPEC = SIXTH / "specs" / "run-065-adaptive-exact-cuts.json"
RESCUE_SPEC = SIXTH / "specs" / "run-065-r1-refine-one-cell.json"


class Run065RefineContractTest(unittest.TestCase):
    def test_exact_partition_identity_and_replacement(self):
        ...
        self.assertEqual(children, [62, 126, 190, 254])
        self.assertTrue(all(child % 64 == 62 for child in children))
        ...
        with tempfile.TemporaryDirectory(dir=SIXTH) as temporary:
            ...
            self.assertEqual(outputs["child_shards"], "62 126 190 254")
            self.assertEqual(outputs["parent_workflow_run"], "31404374848")
            self.assertEqual(outputs["parent_logical_run"], "run-065")
            self.assertEqual(outputs["graph_id"], "c4c4")
            self.assertIn('"cut_version": "4"', result.stdout)
