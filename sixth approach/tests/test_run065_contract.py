#!/usr/bin/env python3
"""Synthetic contract tests for grouped version-4 exact-cut shards."""
...
SPEC = SIXTH / "specs" / "run-065-adaptive-exact-cuts.json"
BUNDLE = SIXTH / "specs" / "run-065-exact-event-cuts.json"
HEADER = SIXTH / "run052_adaptive" / "exact_event_cuts_v4.h"
...
class Run065ContractTest(unittest.TestCase):
    def test_cut_table_complete_matrix_archive_and_unknown_rejection(self):
        spec = json.loads(SPEC.read_text(encoding="utf-8"))
        self.assertEqual(spec["run_id"], "run-065")
        self.assertEqual(spec["exact_cut_version"], 4)
        ...
        run(SIXTH / "run065_cut_contract.py", "--bundle", BUNDLE, "--header", HEADER)
        ...
        self.assertEqual(bundle["base_version"], 3)
        self.assertEqual(len(bundle["cuts"]), 396)
        self.assertEqual(
            {graph: sum(cut["graph"] == graph for cut in bundle["cuts"])
             for graph in ("C8", "C5+C3", "C4+C4")},
            {"C8": 125, "C5+C3": 139, "C4+C4": 132},
        )
        ...
        with tempfile.TemporaryDirectory(dir=SIXTH) as temporary:
            ...
            for graph in spec["graphs"]:
                ...
                for shard in range(spec["shards_per_graph"]):
                    ...
                    (graph_root / f"shard-{shard}.exit").write_text("0\n", encoding="utf-8")
